...
int main() {
  std::cout << "\n'make experiment' will run this main" << std::endl;
  ...
  std::vector<CardT> deck_stack = board.get_deck().toSeq();

  for (int k = 0; k < 64; k++){
    std::cout << deck_stack[k].s << " " << deck_stack[k].r << "\n";

  }
  //std::cout << board.get_deck().top().s << " " << board.get_deck().top().r << "\n";


  
  try {
    std::cout << "\nValid tab move? " << board.is_valid_tab_mv(Foundation, 0, 0) << std::endl;
